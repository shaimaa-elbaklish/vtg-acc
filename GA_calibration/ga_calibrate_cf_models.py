...
from calibrator import GACalibrationConfig
...
filenames = [FILES_ACC_HUMAN[TRAIN_ACC_IDX], FILES_ACC_HUMAN[TEST_ACC_IDX]]
...
params = {
    'desired_speed': {'value': 30.0, 'min': 30.0, 'max': 35.0},
    ...
    'delta': {'value': 4.0, 'min': 0.1, 'max': 10, 'vary': False},
    'max_accel_idm': {'value': 0.3, 'min': 0.1, 'max': 5},
    'comfort_decel': {'value': 3.0, 'min': 0.1, 'max': 5}
}


calib_config = GACalibrationConfig(
    parameters = params,
...
for c in range(1, num_veh-1):
    ...
    out_res, out_params = calib.fit(
        ...
        time=train_data_dict['time'], verbose=True, validate=True
    )

    cf_model.set_params_from_dict(out_params, is_ga=True)
    cf_model.save_params_to_json(f"./calibration_results/GA/AstaZero_Human_{train_data_dict['veh_type']}_IDM_best_fit.json", np.mean(out_res))
